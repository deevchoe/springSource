...
public class Springweb19jpaJoinApplication {
	...
	public void initMembers() {
		...
		try {
			...
			for(String que:queries) {
				em.createNativeQuery(que).executeUpdate();	// insert니까 executeUpdate!
			}
			tx.commit();
		} catch (Exception e) {
		...
		// 40번 부서의 다른 정보를 이용해 직원 정보를 저장할 것이 아니면 find 없이
		// 부서번호만 Dept 객체에 넣는다.
	...
	public static void main(String[] args) {
		SpringApplication.run(Springweb19jpaJoinApplication.class, args);
	}
